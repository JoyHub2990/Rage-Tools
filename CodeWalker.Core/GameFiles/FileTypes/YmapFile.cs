...
//#endif
        ...
        public void EnsureChildYmaps(GameFileCache gfc)
        {
            ...
            {
                //a ymap with child nodes but no entities of its own (a _lodlights file, an
                //occluder-only file) has RootEntities == null - it threw here and its node died
                List<YmapEntityDef> newroots = new List<YmapEntityDef>(RootEntities ?? Array.Empty<YmapEntityDef>());
                for (int i = 0; i < ChildYmaps.Length; i++)
                {
                    var cmap = ChildYmaps[i];
                    if (cmap == null) continue; //nothing here..
    ...
    public class YmapLODLights
    {
        ...
        public void Init(YmapDistantLODLights parent)
        {
            ...
            CalcBB();
            //a _lodlights file whose parent carries no positions builds no lights; a BVH over a
            //null list threw here and took the whole ymap down with it (its node was left "failed")
            if (LodLights != null && LodLights.Length > 0) BuildBVH();
        }
